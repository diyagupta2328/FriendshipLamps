/*
 * ============================================================
 * FRIENDSHIP LAMP — ESP32 Firmware for LAMP 1
 * ============================================================
 *
 * WORKFLOW:
 *   1. Receives "BTN\r\n" from MSPM0 via UART
 *   2. Publishes "PRESS" to MQTT friendship-lamp/lamp1
 *   3. When "PRESS" arrives on friendship-lamp/lamp2:
 *      → Sends "ACTIVE\r\n" to MSPM0 (lights LEDs for 15s)
 *      → Plays melody via I2S for 15 seconds
 *
 * WIRING:
 *   ESP32 GPIO3 (RX) ← MSPM0 PA10 (TX)
 *   ESP32 GPIO1 (TX) → MSPM0 PA11 (RX)
...
const char* TOPIC_PUBLISH   = "friendship-lamp/lamp1";
const char* TOPIC_SUBSCRIBE = "friendship-lamp/lamp2";
...
/* ── Music duration matches LED duration ─────────────────── */
#define MUSIC_DURATION_MS 15000
...
/* ────────────────────────────────────────────────────────── */
/* MQTT CALLBACK                                              */
/* Called when partner (Lamp 2) pressed their button         */
/* ────────────────────────────────────────────────────────── */
...
void connectMQTT()
{
    while (!mqttClient.connected())
    {
        ...
        String clientId = "lamp1-" + WiFi.macAddress();
        clientId.replace(":", "");
...
void setup()
{
    /* UART0 — GPIO1(TX) and GPIO3(RX) — communicates with MSPM0 */
    Serial.begin(115200);
    delay(500);
...
void loop()
{
    ...
    while (Serial.available())
    {
        ...
        if (c == '\n')
        {
            ...
            if (uartBuf == "BTN")
            {
                Serial.println("[ACTION] BTN detected — publishing PRESS to MQTT...");
                sendButtonFlag = true;
            }
            uartBuf = "";
        }
